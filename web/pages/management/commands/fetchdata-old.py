...
def Populate():
    ...
    # Set most_current attribute to false on all objects
    CE.objects.all().update(most_current_record=False)
    Site.objects.all().update(most_current_record=False)
    ...
    for x in range(len(data)):
        new_ce_id = get_if_available('ceid', data[x])
        existing_CE = CE.objects.filter(ce_id=new_ce_id) 
        if len(existing_CE) == 1:
            existing_CE.most_current_record = True
        else:
